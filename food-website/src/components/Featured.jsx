...
const Featured = () => {  
  ...
  return (
    <div className=' max-w-[1520px] h-[500px] w-full py-4 px-4 relative group'>
        ...
        <div className='hidden group-hover:block absolute bg-orange-500 top-[50%]  z-10 left-5  text-2xl rounded-full p-2 text-white cursor-pointer  -translate-x-0 translate-y-[-50%]' onClick={prevslider}>
            <BsChevronCompactLeft />
        </div>
        <div className='hidden group-hover:block absolute bg-orange-500 top-[50%]  z-10 right-5 -translate-x-0 translate-y-[-50%] text-2xl rounded-full p-2 text-white cursor-pointer' onClick={nextSlider}>
            <BsChevronCompactRight />
        </div> 
        <div className='flex top-4 justify-center py-2'>{
            sliders.map((slideItem,slideIndex)=>(
                <div key={slideIndex} className="text-2xl cursor-pointer" onClick={()=>moveToNextSlide(slideIndex)}><RxDotFilled/></div>
            ))
        }
